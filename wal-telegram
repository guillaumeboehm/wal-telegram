...
prepare() {
  ...
  if [[ "$bg" == "wal" ]]; then # if no background is specified
    if [[ "$mode" == wal ]] && [[ "$bg_mode" == "background" ]]; then
      ...
      bg_file="${bg_path##*/}"
      bg_file="${bg_file#.}" # if there is a '.' before the name
      bg_ext="${bg_file##*.}"
      if [[ "$bg_ext" == "$bg_file" ]]; then # if the file has no extension
        if [[ $(file "$bg_path" | cut -d' ' -f2) = "PNG" ]]; then
          bg_ext="png"
        else
          bg_ext="jpg" # fallback on jpg
        fi
      fi
